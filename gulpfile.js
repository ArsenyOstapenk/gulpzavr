...
task('copy:libs', () => {
   return src(`${SRC_PATH}/libs/**/*`).pipe(dest(`${DIST_PATH}/libs`))
});
...
task(
   'default',
   series('clean', parallel('pug', 'copy:html', 'copy:video', 'copy:fonts', 'copy:libs', 'copy:img', 'styles', 'scripts'), parallel('watch','server'))
